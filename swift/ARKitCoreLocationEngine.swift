...
class ARKitCoreLocationEngineImpl: NSObject, ARKitCoreLocationEngine {
    let barometrLabel: UILabel

    init(view: SCNView, locationManager: LocationManager, locationEstimatesHolder: LocationEstimatesHolder, barometrLabel: UILabel) {
        self.scnView = view
        self.locationManager = locationManager
        self.locationEstimatesHolder = locationEstimatesHolder
        self.barometrLabel = barometrLabel
        super.init()
...
extension ARKitCoreLocationEngineImpl: LocationManagerListener {
    ...
    func onLocationUpdate(_ location: CLLocation) {
        barometrLabel.text = "Level: \(Int((location.altitude - 349)/3)+1)/\(Int(location.altitude))m"
        
        //barometrLabel.text = "Level: \(Int(location.altitude))"
        guard let positionOnScene = currentScenePosition() else { return }
